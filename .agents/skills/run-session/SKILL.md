...
3. Otherwise run `python3 scripts/learning.py plan-session-candidates --minutes <minutes>`. Use its review budget, due Reviews, Practice Units, availability, primary focus, fit, and priority to propose a compact plan. A suggested action marked `partial: true` is valid when the available time meets `min_partial_unit_minutes`; omit the planner-only `partial` fields when calling `create-session`.
...
7. Load and follow `.agents/skills/assess-answer/SKILL.md` for each new Evidence. After completed actions have Evidence and Assessment, run `update-progress`, then complete the Session with every new Evidence ID in actual execution order. A Session may also complete without Evidence when the user stops before any action becomes `in_progress`.
