...
Use the script output as the factual status; do not reconstruct state from chat memory. Keep Unit mastery and review scheduling distinct: the only mastery statuses are `learning`, `practice`, and `verified`, while due dates and intervals live under `Progress.review`. You may add a brief interpretation but must not invent Progress, mastery, Reviews, or Sessions absent from repository files.
