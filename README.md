...
`state` reports `uninitialized`, `partial`, or `initialized`. Validation is read-only. Evidence and Assessment Events are create-only, Session checkpoint state is atomically replaceable, Progress is rebuildable, and a repeated `init` must not overwrite an initialized topic or existing Primary Data. One repository may contain at most one Session whose status is `active` or `paused`.
